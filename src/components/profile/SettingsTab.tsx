...
export default function SettingsTab({ 
  ...
  async function handleUnlinkProvider(provider: string) {
    ...
    setIsSubmitting(true)
    try {
      await unlinkProviderMutation({ provider })
    } catch (e: any) {
      alert(e.message || "Failed to disconnect provider")
    } finally {
      setIsSubmitting(false)
    }
  }
  ...
  async function handleLinkGoogle() {
    setIsSubmitting(true)
    try {
      await signIn("google", { redirectTo: window.location.origin + "/profile" })
    } catch (e: any) {
  ...
  const linkedProviders = authStatus?.linkedProviders ?? []
  const hasPasswordAuth = linkedProviders.includes("password")
  const hasGoogleAuth = linkedProviders.includes("google")
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="text-xs font-bold text-text-muted uppercase italic">
                  {hasGoogleAuth ? 'GOOGLE_OAUTH_CONNECTED' : 'GOOGLE_OAUTH_NOT_LINKED'}
                </div>
              </div>
            </div>
            {isAuthLoading ? (
              ...
              <button 
                ...
                disabled={isSubmitting || linkedProviders.length <= 1}
                className="text-[9px] font-black transition-colors text-obsidian/20 hover:text-red-600 uppercase italic flex items-center gap-1 disabled:opacity-30 disabled:cursor-not-allowed"
              >
              ...
              <button 
                ...
                className="bg-obsidian text-accent px-4 py-2 text-[9px] font-black italic uppercase transition-all flex items-center gap-2 hover:scale-105"
              >
                LINK ACCOUNT <ExternalLink className="w-3 h-3" />
              </button>
            )}
          </div>
          ...
          <div className={`bg-white border-2 border-obsidian p-5 flex items-center justify-between group hover:bg-workspace transition-all ${!hasPasswordAuth && !isAuthLoading ? 'opacity-50 grayscale' : ''}`}>
            ...
            {isAuthLoading ? (
              ...
              <>
                ...
                {!hasPasswordAuth && !isAuthLoading && (
                  <button 
                    onClick={() => setIsPasswordModalOpen(true)}
                    className="px-4 py-2 bg-obsidian text-accent text-[9px] font-black italic uppercase hover:scale-105 transition-all flex items-center gap-2"
                  >
                    ADD PASSWORD <Plus className="w-3 h-3" />
                  </button>
                )}
              </>
            )}
